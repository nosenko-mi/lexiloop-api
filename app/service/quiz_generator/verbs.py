...
def check_negative(target_idx: int, tags: tuple) -> bool:
...
def generate_tense_from_tag(tag: str, verb: str) -> tuple[bool, str]:
    __pattern_stopiteration_workaround()
...
def __map_to_tense(verb: str, tag: str) -> str:
    lem = lemma(verb)

    return conjugate(lem, **tag_to_verb_map[tag])
